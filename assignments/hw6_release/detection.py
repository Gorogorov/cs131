...
def sliding_window(image, template_feature, step_size, window_size, pixel_per_cell=8,
    ...
    for r in range(0, H, step_size):
        for c in range(0, W, step_size):
            score = 0
            ### YOUR CODE HERE
            window_hog = feature.hog(pad_image[r:r+winH, c:c+winW],
                                     pixels_per_cell=(pixel_per_cell, pixel_per_cell),
                                     block_norm='L1')
            score = np.sum(window_hog * template_feature)
            if max_score < score:
                max_score = score
                maxr = r
                maxc = c
...
def pyramid(image, scale=0.9, min_size=(200, 100)):
    ...
    while True:
        ...
        ### YOUR CODE HERE
        if min_size[0] > image.shape[0] and min_size[1] > image.shape[1]:
            break
        ### END YOUR CODE
...
def pyramid_score(image, template_feature, shape, step_size=20,
    ...
    ### YOUR CODE HERE
    for image in images:
        score, r, c, response_map = sliding_window(image[1], template_feature, step_size, shape, pixel_per_cell=pixel_per_cell)
        if max_score < score:
            max_score = score
            maxr = r
            maxc = c
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    maxr, maxc = np.argmax(heatmap) // heatmap.shape[1], np.argmax(heatmap) % heatmap.shape[1]    
    ### END YOUR CODE
